...
@pytest.mark.django_db()
def test_submit_other_student_student_2(api_client, student_2):
...
# @pytest.mark.django_db()
# def test_empty_content_assignment_create_student_1(api_client, student_1):
#     content = 'ABCD TESTPOST'
...
#     assert error == ['No Content in the assignment']
...
@pytest.mark.django_db()
def test_student_set_grade_student_1(api_client, student_1):
    response = api_client.patch(
        reverse('students-assignments'),
        data=json.dumps({
            'id': 2,
...
@pytest.mark.django_db()
def test_student_set_state_graded_student_1(api_client, student_1):
    response = api_client.patch(
        reverse('students-assignments'),
        data=json.dumps({
            'id': 2,
